...
void func(char* cipher_l,char* cipher_r,char* expand_cipher,char* key,int sbox[8][64]){
    ...
    //xor with cipher_left
    for(int j=0;j<32;j++){
        tmp[j] = (permu[j] xor cipher_l[j] )+'0';
    }
    //exchange L and R
    for(int i=0;i<32;i++){
        cipher_l[i] = cipher_r[i];
    }
    for(int i=0;i<32;i++){
        cipher_r[i] = tmp[i];
    }
...
int main(void){
    ...
    char plaintext[]= "0110110001100101011000010111001001101110011010010110111001100111" ;
    char ciphertext[64]  ;
    ...
    strncpy(cipher_l, tmp, 32);
    strncpy(cipher_r,&tmp[32],32);
    ...
    for (int round=1; round<=16; round++) {
        ...
        product_encry(cipher_r, e, expand_cipher, 48);
        ...
        if (offset ==1) {
            //rotation key_l
            tmp_key[0]=key_l[0];
            strncpy(key_l, &key_l[offset], 28-offset);
            key_l[27] =tmp_key[0];
            //rotation key_r
            tmp_key[0]=key_r[0];
            strncpy(key_r, &key_r[offset], 28-offset);
            key_r[27] =tmp_key[0];
        }
        else if (offset == 2) {
            //rotation key_l
